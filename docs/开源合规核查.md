...
> 核查时间：2026-09-18　核查人：开发会话（AI 辅助）
> 触发：用户要求确认"界面部分是否直接使用了 MAA / MaaEnd 的代码"，以决定发行许可与鸣谢措辞。
...
2. 参考的是**视觉设计语言**（配色、圆角、深色面板、无边框布局），实现代码为本项目原创；
3. 因此：
   - 鸣谢措辞应写为**"界面视觉风格参考自 MAA / MaaWpfGui 与 MaaEnd"**，**不得**写成"使用了其代码"；
