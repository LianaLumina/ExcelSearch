...
class Win32Acrylic
{
	...
	ComPtr<ISpriteVisual>				m_rootVisual{nullptr};
	// 当前效果参数（供 SetBlurAmount 重建 brush 时保留 tint/luminosity）
	D2D1_COLOR_F						m_tintColor{0.125f, 0.125f, 0.125f, 0.4f};
	D2D1_COLOR_F						m_luminosityColor{0.125f, 0.125f, 0.125f, 0.8f};
	float								m_blurAmount{30.f};
	bool								m_legacy{false};
public:
	Win32Acrylic(HWND hWnd)
	{
		...
		ThrowIfFailed(
			m_compositor->CreateSpriteVisual(&m_rootVisual)
		);
		ThrowIfFailed(
			TryAs<IVisual2>(m_rootVisual)->put_RelativeSizeAdjustment({1.f, 1.f})
		);
		ThrowIfFailed(
			m_rootVisual->put_Brush(CreateAcrylicBrush(m_tintColor, m_luminosityColor, m_blurAmount).Get())
		);

		// now set it as root visual
		ThrowIfFailed(
			TryAs<ICompositionTarget>(m_target)->put_Root(TryAs<IVisual>(m_rootVisual).Get())
		);
	}
	~Win32Acrylic() = default;

	// V0.3.0 增强：动态调整毛玻璃强度（重建 brush 并应用）
	void SetBlurAmount(float blurAmount)
	{
		if (blurAmount < 0.f) blurAmount = 0.f;
		if (blurAmount > 100.f) blurAmount = 100.f;
		m_blurAmount = blurAmount;
		if (m_rootVisual)
		{
			ComPtr<ICompositionBrush> brush =
				m_legacy
					? CreateAcrylicBrush_Legacy(m_tintColor, m_blurAmount)
					: CreateAcrylicBrush(m_tintColor, m_luminosityColor, m_blurAmount);
			ThrowIfFailed(m_rootVisual->put_Brush(brush.Get()));
		}
	}

	// V0.3.0 增强：切换 tint 色（深色/浅色模式联动）
	void SetTintColor(D2D1_COLOR_F tintColor, D2D1_COLOR_F luminosityColor)
	{
	...
	// V0.3.0 增强：显式指定 Win10 legacy 配方（Win11 自动用标准配方）
	void UseLegacy(bool legacy) { m_legacy = legacy; }
	...
	//
	//      </BlendEffect>
	ComPtr<ICompositionBrush> CreateAcrylicBrush(D2D1_COLOR_F tintColor = {0.125f, 0.125f, 0.125f, 0.4f}, D2D1_COLOR_F luminosityColor = {0.125f, 0.125f, 0.125f, 0.8f}, float blurAmount = 30.f)
	{
		// border effect
	...
	ComPtr<ICompositionBrush> CreateAcrylicBrush_Legacy(D2D1_COLOR_F tintColor = {0.125f, 0.125f, 0.125f, 0.4f}/*{0.95f, 0.95f, 0.95f, 0.4f}*/, float blurAmount = 30.f)
	{
		// border effect
		auto borderEffect{Make<BorderEffect>()};
		borderEffect->SetExtendX(D2D1_BORDER_EDGE_MODE_WRAP);
